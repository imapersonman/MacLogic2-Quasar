...
describe('unelaborate', () => {
    ...
    test('?X', () => expect(unelaborate(mv('X'))).toEqual(['X']))
...
describe('elaborate_sequent', () => {
    ...
    test('A, Aab ⊢ C elaboration error in assumptions', () => expect(
        elaborate_sequent([['A'], ['A', 'a', 'b']], ['C'])
    ).toEqual(
        new ErrorInAssumptions(new IncompatibleCtxs(mk_map(['A', o]), mk_map(['A', pred(2)], ['a', i], ['b', i]), A, o, pred(2)), 1)
    ))

    test('Aab ⊢ A elaboration error in conclusion', () => expect(
        elaborate_sequent([['A', 'a', 'b']], ['A'])
    ).toEqual(
        new ErrorInConclusion(new IncompatibleCtxs(mk_map(['A', pred(2)], ['a', i], ['b', i]), mk_map(['A', o]), A, pred(2), o))
    ))
})
