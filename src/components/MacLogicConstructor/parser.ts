...
import { is_array, is_empty, is_object, is_string } from 'coastline/src/utilities'
...
const wrap = (p: P.Parser<unknown>): P.Parser<unknown> =>
    P.alt(p.wrap(P.string('('), P.string(')')), p.wrap(P.string('['), P.string(']')), p.wrap(P.string('{'), P.string('}')))

const LFOL = P.createLanguage({
    Expression: (r) => P.alt(r.And, r.Or, r.Implies, r.Iff, r.Factor/*, r.Individual*/).wrap(P.optWhitespace, P.optWhitespace),
    Factor: (r) => P.alt(r.Exists, r.Forall, r.Not, r.Predicate, wrap(r.Expression)),
    Absurd: () =>
        P.alt(
            P.string('⊥'),
            P.string('\\F'),
    ...
    Not: (r) => P.alt(P.string('~'), P.string('¬'), P.string('-')).skip(P.optWhitespace).then(r.Factor).map((f) => ['~', f]),
    ...
    Forall: (r) => P.alt(
        P.seq(P.alt(P.string('\\A'), P.string('\\U'), P.string('∀')), r.Individual, r.Factor).map(([, x, f]) => ['∀', x, f]),
        P.seq(P.seq(P.alt(P.string('\\A'), P.string('\\U'), P.string('∀')), r.Individual).wrap(P.string('('), P.string(')')), r.Factor).map(([[, x], f]) => ['∀', x, f])
    ),
    Exists: (r) => P.alt(
        P.seq(P.alt(P.string('\\E'), P.string('∃')), r.Individual, r.Factor).map(([, x, f]) => ['∃', x, f]),
        P.seq(P.seq(P.alt(P.string('\\E'), P.string('∃')), r.Individual).wrap(P.string('('), P.string(')')), r.Factor).map(([[, x], f]) => ['∃', x, f])
    )
})
