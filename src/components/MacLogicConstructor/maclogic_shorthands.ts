...
import { app, con, flapp, iv, la, ov, pi } from 'coastline/src/lambda_pi/shorthands'

export const [i, o] = [con('i'), con('o')]
export const absurd = con('absurd')
export const taut = con('taut')
...
export const propositioni = (to_prove: Ast, fp: Ast) => flapp(con('propositioni'), to_prove, fp)
...
export const ifun = (n: number): Ast =>
    n <= 0 ? i
    : pi(iv(n - 1), i, ifun(n - 1))
export const prop = (n: number): Ast =>
    n <= 0 ? o
    : pi(iv(n - 1), o, prop(n - 1))